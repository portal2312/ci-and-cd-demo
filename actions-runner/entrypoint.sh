...
# NOTE: 환경 변수 확인하기
if [ -z "$RUNNER_URL" ] || [ -z "$RUNNER_TOKEN" ] || [ -z "$RUNNER_NAME" ] || [ -z "$RUNNER_WORKDIR" ]; then
  echo "[entrypoint] Check RUNNER_URL:${RUNNER_URL}, RUNNER_TOKEN:${RUNNER_TOKEN}, RUNNER_NAME:${RUNNER_NAME}, RUNNER_WORKDIR:${RUNNER_WORKDIR} environment variables"
  exit 1
fi
